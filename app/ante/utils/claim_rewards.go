...
func ClaimStakingRewardsIfNecessary2(
	ctx sdk.Context,
...
func ClaimStakingRewardsIfNecessary(
	...
	if !found {
		return errortypes.ErrInsufficientFee.Wrapf(
			"%s wrong fee denomination; got: %s; required: %s", addr, amount, stakingDenom,
		)
	}
	...
	ctx.Logger().Debug("---------------:", "addr", addr, "amount", amount, "balance", balance)
	// check if the account has enough balance to cover the fees
	if balance.IsGTE(amountInStakingDenom) {
		return nil
	}
